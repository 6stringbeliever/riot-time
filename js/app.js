...
riot$1.tag2('project-form', '<div class="card" if="{display}"> <project-list projects="{opts.projects}"></project-list> <project-add-form projects="{opts.projects}"></project-add-form> </div>', '', 'class="component"', function(opts) {
    ...
    var tag$$1 = this;
    tag$$1.display = false;
    tag$$1.updateRoute = updateRoute.bind(this);
    ...
    route(tag$$1.updateRoute);
});

riot$1.tag2('project-add-form', '<h2>Add a project</h2> <label htmlfor="proj-add">Name</label> <input type="text" name="proj-add" ref="projAdd" placeholder="Enter your project name"> <button type="button" onclick="{addItem}">Add Project</button>', '', '', function(opts) {
    var tag$$1 = this;
    tag$$1.addItem = addItem;

    function addItem(e) {
      var text = tag$$1.refs.projAdd.value;
      if (text) {
        tag$$1.opts.projects.trigger('project-add', text);
      }
      tag$$1.refs.projAdd.value = '';
    }
});

riot$1.tag2('project-list', '<h2>Projects</h2> <ul show="{this.projects.length > 0}"> <li each="{this.projects}">{name} <button type="button" onclick="{parent.removeItem}">Delete</button></li> </ul> <p show="{this.projects.length === 0}">You haven\'t created any projects yet.</p>', '', '', function(opts) {
    var tag$$1 = this;

    tag$$1.projects = [];

    tag$$1.removeItem = removeItem;

    tag$$1.opts.projects.on('project-update', tag$$1.update);
    tag$$1.on('update', function() {
      tag$$1.projects = tag$$1.opts.projects.getProjects();
    });

    function removeItem(e) {
      var index = tag$$1.projects.indexOf(e.item);
      tag$$1.opts.projects.trigger('project-remove', tag$$1.projects[index].key);
    }

});

riot$1.tag2('entry-form', '<div class="card" if="{display}"> <h2>{header}</h2> <form action="" onsubmit="{submitTime}" ref="form"> <label for="time-date">Date</label> <input name="time-date" ref="date" riot-value="{entry.date}" type="{\'date\'}"> <label for="time-proj">Select your project</label> <select id="projects" ref="project"> <option each="{this.projects}" riot-value="{key}" selected="{entry.key === key}">{name}</option> </select> <label for="time-entry">Enter your hours</label> <input step="0.25" name="time-entry" ref="hours" placeholder="Enter your hours" riot-value="{entry.hours}" type="{\'number\'}"> <label for="time-desc">Description</label> <textarea name="time-desc" rows="4" cols="80" ref="description">{entry.description}</textarea> <label for="time-bill-status"><input type="checkbox" name="time-bill-status" ref="billStatus" checked="{entry.billed}"> Billed</label> <button type="submit">{btntext}</button> <a if="{key}" onclick="{cancel}">Cancel</a> </form> </div>', '', 'class="component"', function(opts) {

    var tag$$1 = this;
    tag$$1.display = true;
    tag$$1.projects = [];
    tag$$1.errors = [];
    tag$$1.submitTime = submitTime;
    tag$$1.reset = reset;
    tag$$1.cancel = cancel;
    tag$$1.entry = {};
    tag$$1.updateRoute = updateRoute.bind(this);

    tag$$1.on('mount', function() {
      tag$$1.reset();
    });
    ...
    tag$$1.on('update', function() {
      tag$$1.projects = tag$$1.opts.projects.getProjects();
      tag$$1.header = tag$$1.key ? 'Edit Entry' : 'Enter Time';
      tag$$1.btntext = tag$$1.key ? 'Update Time' : 'Enter Time';
    });

    tag$$1.opts.state.on('update-edit-entry', function() {
      tag$$1.reset();
      tag$$1.key = tag$$1.opts.state.editEntry;
      tag$$1.entry = tag$$1.opts.timeEntry.getEntryForKey(tag$$1.key);
      tag$$1.update();
    });
    ...
    function submitTime(e) {
      ...
      if (tag$$1.errors.length === 0) {
        if (tag$$1.key) {
          tag$$1.opts.timeEntry.trigger('time-update', tag$$1.key, entry);
        } else {
          tag$$1.opts.timeEntry.trigger('time-add', entry);
        }
        tag$$1.reset();
      }
    }
    ...
    function reset() {
      tag$$1.refs.form.reset();
      tag$$1.entry = getEmptyEntry();
    }
...
riot$1.tag2('time-report', '<div class="card" if="{display}"> <h2>Time</h2> <label for="proj-select">Project</label> <select name="proj-select" ref="projSelect" onchange="{getTime}"> <option value="all">All</option> <option each="{this.projects}" riot-value="{key}">{name}</option> </select> <label for="proj-bill-status">Billing Status</label> <select name="proj-bill-status" ref="projBillStatus" onchange="{getTime}"> <option value="all">All</option> <option value="false">Unbilled</option> <option value="true">Billed</option> </select> <label><input type="checkbox" ref="selectall" onchange="{selectAll}"> Select All</label> <button type="button" name="proj-bill" onclick="{billEntries}">Bill Selected</button> <ul if="{time}"> <time-item each="{time}" data="{this}"></time-item> </ul> <div if="{!time}"> <p>No time entries found. </div> </div>', '', 'class="component"', function(opts) {
    ...
    tag$$1.projects = [];
    tag$$1.time = [];
    tag$$1.getTime = getTime;
    tag$$1.getName = getName;
    tag$$1.editEntry = editEntry;
    tag$$1.deleteEntry = deleteEntry;
    tag$$1.billEntries = billEntries;
    tag$$1.selectAll = selectAll;
    tag$$1.select = select;
    tag$$1.updateRoute = updateRoute.bind(this);

    tag$$1.opts.projects.on('project-update', function() {
      tag$$1.projects = tag$$1.opts.projects.getProjects();
      tag$$1.update();
    });

    tag$$1.opts.timeEntry.on('time-update', tag$$1.getTime);

    tag$$1.opts.timeEntry.on('time-query-returned', function() {
      tag$$1.time = tag$$1.opts.timeEntry.timeEntries;
      tag$$1.update();
    });

    function getTime() {
      tag$$1.opts.timeEntry.trigger('time-retrieve', tag$$1.refs.projSelect.value, tag$$1.refs.projBillStatus.value);
    }

    function getName(val) {
      return tag$$1.opts.projects.getProjectNameFromKey(val);
    }

    function editEntry(val) {
      tag$$1.opts.state.trigger('set-edit-entry', val.item.key);
    }

    function deleteEntry(val) {
      tag$$1.opts.timeEntry.trigger('time-remove', val.item.key);
    }
    ...
    function selectAll(val) {
      tag$$1.time.forEach(function(time) {
    ...
    function billEntries() {
      tag$$1.time.forEach(function(time) {
        if (time.selected) {
          tag$$1.opts.timeEntry.trigger('time-bill', time.key);
          time.selected = false;
        }
      });
    }
...
riot$1.tag2('time-item', '<li class="{billed: billed}"><input type="checkbox" checked="{selected}" onchange="{parent.select}"> {parent.getName(project)} {date} - {hours} - {description} <button type="button" onclick="{parent.editEntry}">Edit</button> <button type="button" onclick="{parent.deleteEntry}">Delete</button></li>', '', '', function(opts) {
});

riot$1.tag2('app-nav', '<nav> <ul class="header-nav"> <li class="header-nav-item" each="{links}"><a href="{\'#\' + url}">{text}</a></li> </ul> </nav>', '', '', function(opts) {
    var tag$$1 = this;
...
var projects = new ProjectList(firebase.database(), riot$1);
var timeEntry = new TimeEntryList(firebase.database(), riot$1);
var state = new State(riot$1);
...
riot$1.mount('project-form', {projects: projects});
riot$1.mount('project-add-form', {projects: projects});
riot$1.mount('project-list', {projects: projects});
riot$1.mount('entry-form', {projects: projects, timeEntry: timeEntry, state: state});
riot$1.mount('time-report', {projects: projects, timeEntry: timeEntry, state: state});
riot$1.mount('time-item');
riot$1.mount('app-nav', {links: links});
